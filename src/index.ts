...
class ClickSendServer {
  ...
  private client: ClickSendClient;
  private lastActionTime: number = 0;
  private readonly RATE_LIMIT_MS = 12000; // 5 actions per minute
  ...
  private checkRateLimit() {
    ...
    if (now - this.lastActionTime < this.RATE_LIMIT_MS) {
      throw new McpError(
        ...
        `Rate limit exceeded. Please wait ${Math.ceil((this.RATE_LIMIT_MS - (now - this.lastActionTime)) / 1000)} seconds.`
      );
    }
